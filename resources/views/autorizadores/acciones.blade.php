  ...
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="exampleModalLabel">Eliminar Autorizador</h5>
      ...
      <div class="modal-body text-center">
      	<div class="container-fluid text-center">
      		...
      		<h5>¿Esta Seguro de Eliminar este Autorizador?</h5>
	      	</div>
	      	<div class="row text-center">
	      		<form>	   	      			
...
<div class="modal fade" id="editarModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="exampleModalLabel">Editar Autorizador</h5>
        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
          <span aria-hidden="true">&times;</span>
        </button>
      </div>
...
<script >
	...
	function editarRegion(){
		...
		ajaxEditar(id,nombre);		
	}
	

	//////////////funciones ajax//////////////
	function ajaxEditar(id,nombre){
	...
	function ajax_getRegion(id,modal){
		$.ajax({
			...
			error(error){				
				alert('Autorizador no Encontrado');
	...
	function ajaxEliminar(id){
		...
		$.ajax({
			...
			success:function(resp){	
				...
				if(resp == 'ok'){
					alertify.set('notifier','position', 'top-right');
					alertify.notify('Autorizador Eliminado', 'success', 3, function(){  console.log(); });  
					location.reload();
				}
			},
			error(error){
				$('#okDeleteLoader').html('<button type="button" id="okDelete" onclick="eliminarRegion()" class="btn btn-primary">Si</button>');
				alertify.set('notifier','position', 'top-right');
				alertify.notify('Este Autorizador no puede ser Eliminado, constituye perdida de Datos', 'error', 6, function(){  console.log(); });  
				$('#deleteModal').modal('hide');
			}
		});
